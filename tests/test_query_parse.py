...
def test_parse_question_team_variants_mlb():
    cases = [
        ("cubs next game", "CHC"),
        ("what are the dodgers odds tonight", "LAD"),
        ("astros next game", "HOU"),
        ("padres next few", "SD"),
    ]

    for question, team in cases:
        intent = parse_question(question)
        assert intent.intent_type in {"team_next_game", "team_next_n_games"}
        assert intent.team == team
        assert intent.league == "MLB"
...
def test_parse_question_team_variants_nba():
    ...
    for question, team in cases:
        intent = parse_question(question, default_league="NBA")
        assert intent.intent_type == "team_next_game"
        assert intent.team == team
...
def test_parse_question_default_league_override():
    mlb_intent = parse_question("Toronto next game")
    ...
    assert mlb_intent.intent_type == "team_next_game"
    assert mlb_intent.team == "TOR"
    assert mlb_intent.league == "MLB"
...
def test_parse_question_championships():
    cases = [
        ...
        ("what are the odds the dodgers win the world series?", "LAD", "MLB", "World Series"),
        ("do the cubs win it all?", "CHC", "MLB", "World Series"),
...
def test_parse_question_college_basketball_respects_supported_leagues():
...
def test_parse_question_league_report():
    ...
    mlb_intent = parse_question("Give me the MLB team report table", default_league="NBA")
    assert mlb_intent.intent_type == "league_report"
    assert mlb_intent.league == "MLB"


def test_parse_question_bet_history_defaults_to_mlb_and_tracks_game_breakdown():
    ...
    assert recap2.league == "MLB"
    assert recap2.history_period == "yesterday"
    assert recap2.include_games is True

    net_profit = parse_question("What was my net profit last night?")
    assert net_profit.intent_type == "bet_history_summary"
    assert net_profit.league == "MLB"
    assert net_profit.history_period == "yesterday"
    assert net_profit.include_games is True
