...
def _default_tune_result(model_name: str) -> dict:
    ...
    best_params = {"c": float(config.default_c)}
...
def quick_tune_penalized_glm(
    ...
    *,
    ...
    l1_ratio_values = list(l1_ratio_grid) if l1_ratio_grid is not None else [0.1, 0.25, 0.5, 0.75]
    ...
    if config.penalty == "elasticnet":
        param_grid = [{"c": float(c), "l1_ratio": float(l1_ratio)} for c in c_values for l1_ratio in l1_ratio_values]
    else:
        param_grid = [{"c": float(c)} for c in c_values]
