"""Experimental two-stage proxy model.

The current implementation uses non-CAS intermediate learners, so it remains
outside the governing theory-core lane.
"""
...
class TwoStageModel(BaseProbModel):
    """Hybrid proxy model that feeds learned intermediate targets into a logit."""
    ...
    def fit(self, df: pd.DataFrame, feature_columns: list[str], target_col: str = "home_win") -> None:
        ...
        inter_preds = []
        for idx, target in enumerate(self.stage1_targets):
            y_target = train[target].fillna(train[target].median()).to_numpy(dtype=float)
            model_key = f"stage_{idx}"
            self.stage1_models[model_key].fit(x, y_target)
    ...
    def predict_intermediates(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        out = {}
        for idx, target in enumerate(self.stage1_targets):
            out[target.replace("target_", "pred_")] = self.stage1_models[f"stage_{idx}"].predict(x)
    ...
    def predict_proba(self, df: pd.DataFrame) -> np.ndarray:
        ...
        probability = self.stage2.predict_proba(z)[:, 1]
