"""Theory-compatible score/count GLM-style bridge model."""
...
class GoalsModelOutput:
    """Single-row score/count bridge output."""
...
class GoalsPoissonModel:
    """Poisson scoring model with a simulated win-probability bridge."""
    ...
    def fit(self, games_df: pd.DataFrame) -> None:
        ...
        for _, row in train.iterrows():
            team_rows.append({"team": row["home_team"], "goals_for": row["home_score"], "goals_against": row["away_score"]})
            team_rows.append({"team": row["away_team"], "goals_for": row["away_score"], "goals_against": row["home_score"]})
        team_games = pd.DataFrame(team_rows)

        grouped = team_games.groupby("team").agg(
            gf=("goals_for", "sum"),
        ...
        for team, row in grouped.iterrows():
            ...
            gf = row["gf"]
            ga = row["ga"]
            self.attack[team] = ((gf + self.shrinkage_alpha * self.league_avg) / (gp + self.shrinkage_alpha)) / max(
                self.league_avg, 1e-6
            )
            self.defense[team] = ((ga + self.shrinkage_alpha * self.league_avg) / (gp + self.shrinkage_alpha)) / max(
                self.league_avg, 1e-6
            )
    ...
    def predict_row(self, home_team: str, away_team: str, n_sim: int = 3000) -> GoalsModelOutput:
        ...
        probability = float(np.mean(home_goals > away_goals) + 0.5 * np.mean(home_goals == away_goals))
        return GoalsModelOutput(
            prob_home_win=float(np.clip(probability, 1e-6, 1 - 1e-6)),
    ...
    def predict_proba(self, games_df: pd.DataFrame, n_sim: int = 3000) -> np.ndarray:
        ...
        for _, row in games_df.iterrows():
            out = self.predict_row(row["home_team"], row["away_team"], n_sim=n_sim)
