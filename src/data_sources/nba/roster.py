...
def _injury_status(player: dict) -> tuple[str | None, str | None]:
    injuries = player.get("injuries") or []
    if not injuries:
        return None, None
    first = injuries[0] or {}
    status = str(first.get("status") or "").strip() or None
    date = str(first.get("date") or "").strip() or None
    return status, date
...
def fetch_roster_snapshot(
    ...
    for team in sorted(set(str(t).upper() for t in team_abbrevs if t)):
        ...
        try:
            payload, raw_path = client.get_json("nba_rosters", url, key=f"{team}_{season_str}")
            raw_paths.append(raw_path)
        except Exception:
            continue

        for player in payload.get("athletes", []):
            injury_status, injury_date = _injury_status(player)
            rows.append(
                {
                    "season": season_str,
                    "team": team,
